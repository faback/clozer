...
class DataManager {
    ...
    func saveLocationRecords (records: [LocationData]) {
        ...
        for eachRecord in records {
            ...
            let Data = NSKeyedArchiver.archivedData(withRootObject: eachRecord)
            NSDataArray.append(Data as NSData)
        }
        ...
        self.defaults.removeObject(forKey: LocationRecords_Key)
        self.defaults.set(self.locationRecords, forKey: LocationRecords_Key)
    ...
    func getUpdatedRecords () -> [LocationData]? {
        ...
        if let RECORDS: [NSData] = self.defaults.array(forKey: LocationRecords_Key) as? [NSData] {
            ...
            for eachRecord in RECORDS {
                ...
                let newRecord = NSKeyedUnarchiver.unarchiveObject(with: eachRecord as Data) as! LocationData
            ...
            locationRecords.sort(by: { $0.timestamp.compare($1.timestamp as Date) == ComparisonResult.orderedAscending})
    ...
    func deleteOldestLocationRecord() {
        ...
        if let fetchedRecords = self.getUpdatedRecords() {
            ...
            sortedRecords.remove(at: 0)
            
            self.saveLocationRecords(records: sortedRecords)
        }
    }
    ...
    func addNewLocationRecord (newRecord: CLLocation) {
        ...
        let lat:NSNumber = NSNumber(value: newRecord.coordinate.latitude)
        let lon: NSNumber = NSNumber(value:newRecord.coordinate.longitude)
        let time: NSDate = (newRecord.timestamp as? NSDate)!
        ...
        //- Sort Records
        sortedRecords.sort(by: { $0.timestamp.compare($1.timestamp as Date) == ComparisonResult.orderedAscending })
    ...
    func updateLocationRecords (newLocation: CLLocation) {
        ...
        if let fetchResults = getUpdatedRecords() {
            ...
            case 0...(self.MaxNoOfRecords-1):
                ...
                addNewLocationRecord(newRecord: newLocation)
        ...
        if let fetchResults = getUpdatedRecords() {
            ...
            for each in fetchResults {
                print(self.fixDateFormat(date: each.timestamp as Date))
            }
        }
        print("-----------")
        //- FOR TESTING above -----
     ...
     ********************************************************************************************************************/
    func fixDateFormat(date: Date) -> NSString {
